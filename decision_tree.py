...
plt.title(f"Ma trận nhầm lẫn trung bình tại k={best_k} (Decision Tree)")
plt.xlabel("Dự đoán")
plt.ylabel("Thực tế")
plt.tight_layout()
plt.show()
...
plt.title(f"Precision, Recall, F1-score tại k={best_k} (Decision Tree)")
plt.ylabel("Giá trị")
plt.ylim(0, 1)
...
plt.plot(range(1, len(last_fold_losses)+1), last_fold_losses, marker='o', label="Decision Tree (k=10)")
plt.title("Log-loss trên từng Fold (Decision Tree, k=10)")
plt.xlabel("Fold")
plt.ylabel("Log Loss")
plt.legend()
plt.grid(True)
