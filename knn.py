...
plt.title(f"Ma trận nhầm lẫn trung bình tại k={best_k} (KNN)")
plt.xlabel("Dự đoán")
plt.ylabel("Thực tế")
plt.tight_layout()
plt.show()
...
plt.title(f"Precision, Recall, F1-score tại k={best_k} (KNN)")
plt.ylabel("Giá trị")
plt.ylim(0, 1)
...
plt.plot(range(1, len(last_fold_losses)+1), last_fold_losses, marker='o', label="KNN (k=10)")
plt.title("Log-loss trên từng Fold (KNN, k=10)")
plt.xlabel("Fold")
plt.ylabel("Log Loss")
plt.legend()
plt.grid(True)
